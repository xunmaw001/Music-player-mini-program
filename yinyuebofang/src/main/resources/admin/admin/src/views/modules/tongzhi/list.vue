        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '通知标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.tongzhiName" placeholder="通知标题" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '通知类型' : ''">
                        <el-select v-model="searchForm.tongzhiTypes" placeholder="请选择通知类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in tongzhiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tongzhi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yinyuebofang/upload/tongzhiMuBan.xls"
                        >批量导入通知数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tongzhi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="tongzhiUploadSuccess"
                                :on-error="tongzhiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tongzhi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tongzhi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="tongzhiUuidNumber"
                                   header-align="center"
                                   label="通知编号">
                        <template slot-scope="scope">
                            {{scope.row.tongzhiUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('tongzhi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              tongzhiTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                tongzhiUuidNumber : null,
                tongzhiName : null,
                tongzhiTypes : null,
                tongzhiContent : null,
                insertTime : null,
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'tongzhi',//当前表表名,
                        sumColum : 'tongzhi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'tongzhi_name',//分组字符串字段
                        // types : 'tongzhi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.tongzhiName!= '' && this.searchForm.tongzhiName!= undefined) {
                    params['tongzhiName'] = '%' + this.searchForm.tongzhiName + '%'
                }
         
                if (this.searchForm.tongzhiTypes!= '' && this.searchForm.tongzhiTypes!= undefined) {
                    params['tongzhiTypes'] = this.searchForm.tongzhiTypes
                }
                        
                params['tongzhiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=tongzhi_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.tongzhiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            tongzhiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tongzhi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
