<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.yinyueName" type="text" placeholder="音乐名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yinyue','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('yinyue','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
	...
	<view style="display: flex;flex-wrap: wrap">
            <uni-drawer ref="showRight" mode="left" :maskClick="true" :width="200" :mask-click="true">
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 50%;">
                            <view :style='yinyueTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in yinyueTypesList" :key="index" :class="yinyueTypes===item.codeIndex?'tabActive':''"
                                  @tap="yinyueTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                        <view style="display: flex;flex-direction: column;width: 50%;">
                            <view v-if="yinyueTypes != 0" :style='yinyueErjiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in yinyueErjiTypesList" :key="index" :class="yinyueErjiTypes===item.codeIndex?'tabActive':''"
                                  @tap="yinyueErjiCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <view class="list"
          :style='{"padding":"20rpx 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"#efefef","borderRadius":0,"borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'
          style="box-sizing: border-box;">
        <view @tap="onDetailTap(product)" class="listm flex flex-between"
              :style='{"padding":"0 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 20rpx 0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","borderRadius":"8rpx","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}'
              v-for="(product,index) in list" :key="index">
            <image
                    :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"8rpx","borderWidth":"0 4rpx","width":"60%","borderStyle":"solid","height":"160rpx"}'
                    mode="aspectFill" class="listmpic"
                    :src="product.yinyuePhoto?baseUrl+product.yinyuePhoto.split(',')[0]:''"></image>
            <view class="listmr">
                <view class="hide2"
                      :style='{"padding":"10rpx 10rpx 0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0 auto","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                    {{product.yinyueName}}
                </view>
                <view v-if="product.yinyueNewMoney"
                      :style='{"padding":"10rpx 10rpx 0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0 auto","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"red","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"34rpx","borderStyle":"solid"}'>
                    ￥{{product.yinyueNewMoney}}
                </view>
                <view v-else-if="product.yinyueValue"
                      :style='{"padding":"10rpx 10rpx 0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0 auto","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                    {{product.yinyueValue}}
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('yinyue','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('yinyue','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 音乐类型 字段 字典表查询方法
            async yinyueTypesSelect(){
                let yinyueTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'yinyue_types',
                });
                yinyueTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.yinyueTypesList = yinyueTypes.data.list
            },
            //当前表的 音乐类型字段类别搜索
			yinyueTypesCategoryClick(categoryName) {
                this.yinyueTypes = categoryName;
                this.yinyueErjiTypes = 0;
                this.yinyueErjiTypesSelect();
                this.mescroll.resetUpScroll();
            },
            async yinyueErjiTypesSelect(){
                let yinyueErjiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'yinyue_erji_types',
                    superId: this.yinyueTypes
                });
                yinyueErjiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.yinyueErjiTypesList = yinyueErjiTypes.data.list
            },
            yinyueErjiCategoryClick(categoryName){
			...
			async upCallback(mescroll) {
                ...
                if (this.yinyueTypes != 0) {
                    params.yinyueTypes = this.yinyueTypes
                }
                if (this.yinyueErjiTypes != 0) {
                    params.yinyueErjiTypes = this.yinyueErjiTypes
                }
                let _this = this
                let res = await this.$api.list(`yinyue`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yinyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yinyueShenheTime=shijian;
                            await _this.$api.update(`yinyue`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.yinyueName) {
                    searchForm['yinyueName'] = this.searchForm.yinyueName
                }

                let res = await this.$api.list(`yinyue`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.yinyueContent!=null){
                        item.yinyueContent = item.yinyueContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
