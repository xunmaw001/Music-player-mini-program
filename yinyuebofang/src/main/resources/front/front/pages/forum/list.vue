	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.forumName" type="text" placeholder="帖子标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('forum','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('forum','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.forumName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('forum','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                <view v-show="item.forumContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">发布内容</text>
                    <view style="width: 70%" v-html="item.forumContent"></view>
                </view>
                 <view v-show="item.superIds" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">父id</text>
                     <text style="width: 70%">{{item.superIds}}</text>
                 </view>
                 <view v-show="item.forumValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">帖子类型</text>
                     <text style="width: 70%">{{item.forumValue}}</text>
                 </view>
                 <view v-show="item.forumStateValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">帖子状态</text>
                     <text style="width: 70%">{{item.forumStateValue}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('forum','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('forum','审核') && item.forumYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
		...
		methods: {
            ...
            async chartDialog() {
                ...
                let res = await _this.$api.requestConditionDataGet("",'newSelectGroupSum',null,{
                    tableName:'forum_shouzhi',
                    groupColumn:'forum_types',
                    // sumColumn:'forum_number',
			...
			async upCallback(mescroll) {
                ...
                res.data.list.forEach(function(item,index){
                    if(item.forumContent!=null){
                        item.forumContent = item.forumContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
                    }
                })
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.forumYesnoTypes=shenheTypes;
                            data.forumShenheTime=shijian;
                            await _this.$api.update(`forum`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.forumName) {
                    searchForm['forumName'] = this.searchForm.forumName
                }

                let res = await this.$api.page(`forum`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.forumContent!=null){
                        item.forumContent = item.forumContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
