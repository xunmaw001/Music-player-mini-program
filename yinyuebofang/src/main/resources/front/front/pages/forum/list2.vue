	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('forum','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
	<view style="">
...
<script>
	export default {
		...
		async onShow() {
            ...
            this.categoryList = forumTypes.data.list

	   		//当前表的 帖子状态 字段 字典表查询方法
		  	let forumStateTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'forum_state_types'
		  	});
		  	this.forumStateTypesList = forumStateTypes.data.list;
				...
				let params = {
                    ...
                    yonghuId:this.userId,//查询自己的
							...
							if (this.categoryName != 0) {
                                params.forumTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`forum`, {
                                id:id,
                            });
                            //await _this.$api.requestConditionDataGet('forum','zhifu',null,{
                            //    id:id
                            //});
