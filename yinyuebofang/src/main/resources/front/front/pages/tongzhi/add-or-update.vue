        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.tongzhiUuidNumber" type="text" v-model="ruleForm.tongzhiUuidNumber" placeholder="通知编号"></input>
            </view>
            <view
                    :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                    class="cu-form-group">
                <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                      class="title">通知标题</view>
                <input   disabled
                         :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                         :disabled="ro.tongzhiName" type="text" v-model="ruleForm.tongzhiName" placeholder="通知标题"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">通知类型</view>
                    <picker @change="tongzhiTypesChange" :value="tongzhiTypesIndex" :range="tongzhiTypesOptions" range-key="indexName">
                        <view
                                :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.tongzhiTypes?ruleForm.tongzhiValue:"请选择通知类型"}}</view>
                    </picker>
                </view>
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.tongzhiContent" v-model="ruleForm.tongzhiContent" placeholder="通知内容"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let tongzhiTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'tongzhi_types',
            }
			let tongzhiTypes = await this.$api.page(`dictionary`, tongzhiTypesParams);
			this.tongzhiTypesOptions = tongzhiTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`tongzhi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.tongzhiId){
                this.ruleForm.tongzhiId = options.tongzhiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.tongzhiUuidNumber)) {
                    this.$utils.msg(`通知编号不能为空`);
                    return
                }
				if ((!this.ruleForm.tongzhiName)) {
                    this.$utils.msg(`通知标题不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`tongzhi`, this.ruleForm);
                } else {
                    await this.$api.save(`tongzhi`, this.ruleForm);
                }
