	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.tongzhiName" type="text" placeholder="通知标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('tongzhi','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('tongzhi','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='tongzhiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in tongzhiTypesList" :key="index" :class="tongzhiTypes===item.codeIndex?'tabActive':''"
                                  @tap="tongzhiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.tongzhiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('tongzhi','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.yonghuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">用户姓名</text>
                    <text style="width: 70%">{{item.yonghuName}}</text>
                </view>
                <!-- 当前表 -->
                 <view v-show="item.tongzhiUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">通知编号</text>
                     <text style="width: 70%">
                         <text v-if="item.tongzhiUuidNumber != null">{{item.tongzhiUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.tongzhiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">通知类型</text>
                     <text style="width: 70%">{{item.tongzhiValue}}</text>
                 </view>
                <view v-show="item.tongzhiContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">通知内容</text>
                    <view style="width: 70%" v-html="item.tongzhiContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('tongzhi','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('tongzhi','审核') && item.tongzhiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 通知类型 字段 字典表查询方法
            async tongzhiTypesSelect(){
                let tongzhiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'tongzhi_types',
                });
                tongzhiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.tongzhiTypesList = tongzhiTypes.data.list
            },
            //当前表的 通知类型字段类别搜索
			tongzhiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.tongzhiTypes != 0) {
                    params.tongzhiTypes = this.tongzhiTypes
                }
                let _this = this
                let res = await this.$api.page(`tongzhi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tongzhi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.tongzhiShenheTime=shijian;
                            await _this.$api.update(`tongzhi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.tongzhiName) {
                    searchForm['tongzhiName'] = this.searchForm.tongzhiName
                }

                let res = await this.$api.page(`tongzhi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.tongzhiContent!=null){
                        item.tongzhiContent = item.tongzhiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
