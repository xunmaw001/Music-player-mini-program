	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('tongzhi', this.tongzhiId);
                    this.detail = res.data
					this.detail.tongzhiContent = this.detail.tongzhiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.tongzhiPhoto ? this.detail.tongzhiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('tongzhiYuyue', {
									yonghuId: _this.user.id,
									tongzhiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/tongzhiYuyue/list');
							}
						}
					});
				},
