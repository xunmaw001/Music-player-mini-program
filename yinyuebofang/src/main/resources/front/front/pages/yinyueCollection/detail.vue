	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yinyueCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('yinyueCollection', this.yinyueCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yinyueCollectionPhoto ? this.detail.yinyueCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yinyueCollectionYuyue', {
									yonghuId: _this.user.id,
									yinyueCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yinyueCollectionYuyue/list');
							}
						}
					});
				},
