	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.gonggaoName" type="text" placeholder="公告名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('gonggao','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('gonggao','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='gonggaoTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in gonggaoTypesList" :key="index" :class="gonggaoTypes===item.codeIndex?'tabActive':''"
                                  @tap="gonggaoTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="news-box3"
      :style='{"padding":"0 10rpx","margin":"0 0 20rpx","borderColor":"red","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
    <view @tap="onDetailTap(product)" v-for="(product,index) in list" :key="index" class="list-item"
        ...
        <view :style='{"padding":"0","boxShadow":"0px 0 0px rgba(153,153,153,1)","margin":"0","borderColor":"#ccc","backgroundColor":"rgba(70, 176, 60, 1)","borderRadius":"50%","borderWidth":"0","width":"16rpx","borderStyle":"solid","height":"16rpx"}'
              class="dian"></view>
        <view :style='{"padding":"0 20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"calc(100% - 40rpx)","lineHeight":"72rpx","fontSize":"28rpx","borderStyle":"solid"}'
              class="title hide1">{{product.gonggaoName}}
        </view>
        <view class="cuIcon-right"
              :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","borderRadius":"0","borderWidth":"0","width":"32rpx","lineHeight":"72rpx","fontSize":"32rpx","borderStyle":"solid"}'></view>
    </view>
</view>	</view>
        <!--   报表    -->
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 公告类型 字段 字典表查询方法
            async gonggaoTypesSelect(){
                let gonggaoTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'gonggao_types',
                });
                gonggaoTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.gonggaoTypesList = gonggaoTypes.data.list
            },
            //当前表的 公告类型字段类别搜索
			gonggaoTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.gonggaoTypes != 0) {
                    params.gonggaoTypes = this.gonggaoTypes
                }
                let _this = this
                let res = await this.$api.list(`gonggao`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gonggao', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.gonggaoShenheTime=shijian;
                            await _this.$api.update(`gonggao`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.gonggaoName) {
                    searchForm['gonggaoName'] = this.searchForm.gonggaoName
                }

                let res = await this.$api.list(`gonggao`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.gonggaoContent!=null){
                        item.gonggaoContent = item.gonggaoContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
