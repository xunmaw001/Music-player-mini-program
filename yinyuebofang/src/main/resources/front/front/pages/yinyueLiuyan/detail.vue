	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yinyueLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('yinyueLiuyan', this.yinyueLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yinyueLiuyanPhoto ? this.detail.yinyueLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yinyueLiuyanYuyue', {
									yonghuId: _this.user.id,
									yinyueLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yinyueLiuyanYuyue/list');
							}
						}
					});
				},
