	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.caozuorizhiText" type="text" placeholder="操作内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('caozuorizhi','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('caozuorizhi','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.caozuorizhiName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('caozuorizhi','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.caozuorizhiThisBiao" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">操作人所在表</text>
                     <text style="width: 70%">
                         <text v-if="item.caozuorizhiThisBiao != null">{{item.caozuorizhiThisBiao}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.caozuorizhiCaozuobiao" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">操作表</text>
                     <text style="width: 70%">
                         <text v-if="item.caozuorizhiCaozuobiao != null">{{item.caozuorizhiCaozuobiao}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.caozuorizhiCaozuozhanghu" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">操作账户</text>
                     <text style="width: 70%">
                         <text v-if="item.caozuorizhiCaozuozhanghu != null">{{item.caozuorizhiCaozuozhanghu}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.caozuorizhiCaozuoleixing" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">操作类型</text>
                     <text style="width: 70%">
                         <text v-if="item.caozuorizhiCaozuoleixing != null">{{item.caozuorizhiCaozuoleixing}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.caozuorizhiText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">操作内容</text>
                    <view style="width: 70%" v-html="item.caozuorizhiText"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('caozuorizhi','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('caozuorizhi','审核') && item.caozuorizhiYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`caozuorizhi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('caozuorizhi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.caozuorizhiShenheTime=shijian;
                            await _this.$api.update(`caozuorizhi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.caozuorizhiThisBiao) {
                    searchForm['caozuorizhiThisBiao'] = this.searchForm.caozuorizhiThisBiao
                }
				if (this.searchForm.caozuorizhiCaozuobiao) {
                    searchForm['caozuorizhiCaozuobiao'] = this.searchForm.caozuorizhiCaozuobiao
                }
				if (this.searchForm.caozuorizhiCaozuozhanghu) {
                    searchForm['caozuorizhiCaozuozhanghu'] = this.searchForm.caozuorizhiCaozuozhanghu
                }
				if (this.searchForm.caozuorizhiCaozuoleixing) {
                    searchForm['caozuorizhiCaozuoleixing'] = this.searchForm.caozuorizhiCaozuoleixing
                }
				if (this.searchForm.caozuorizhiText) {
                    searchForm['caozuorizhiText'] = this.searchForm.caozuorizhiText
                }
				if (this.searchForm.caozuorizhiText) {
                    searchForm['caozuorizhiText'] = this.searchForm.caozuorizhiText
                }

                let res = await this.$api.page(`caozuorizhi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
